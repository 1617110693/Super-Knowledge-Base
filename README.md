...
A local-first desktop knowledge base for AI agents. Built with **Tauri v2 + React + Python**, supports **OpenAI-compatible embedding & rerank models**, **MinerU document parsing**, and ships with an **MCP server** for Claude Code integration.

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Tauri](https://img.shields.io/badge/Tauri-v2-orange)](https://tauri.app/)
[![Python](https://img.shields.io/badge/Python-3.11+-green)](https://python.org)
[![README](https://img.shields.io/badge/README-中文-red)](README.zh-CN.md)

## Features

### Document Management
- **Multi-format**: PDF, DOCX, PPTX, XLSX, images, HTML, Markdown, plain text
- **Auto pipeline**: Upload → MinerU parse → chunk → embed → index — fully automatic
...
- **Hybrid search**: Dense vector + BM25 keyword (FTS) + reranking
...
### MCP Server
12 tools for AI agents — runs as part of the backend, requires app running (or minimized to tray):
...
| `delete_knowledge_base` | Delete a KB and all data |
...
- System tray — close to tray, backend keeps running for MCP
- Sidebar with KB list, Settings panel for API keys and model config
...
### First Launch
1. Go to **Settings** → configure Embedding API and optionally Rerank API
2. Create a **Knowledge Base** → upload documents
3. Search via the UI, or connect **Claude Code** via MCP

## MCP Server Setup

Open the app → Settings → click **"Configure Claude Code MCP"** to auto-generate the config. Or use **"Copy MCP Config"**.
...
│   └── src/                # React frontend
├── services/python-backend/ # Python backend (FastAPI + MCP)
