...
- **LLM**：OpenAI、Azure、Ollama、vLLM 等任意 `/v1/chat/completions` 端点
...
| `llm_api_base` | 对话 LLM API 地址 | `https://api.openai.com` |
| `llm_api_key` | 对话 LLM API 密钥 | (空) |
| `llm_model` | 对话 LLM 模型名称 | `gpt-4o-mini` |
...
MCP Server 让 AI Agent（特别是 **Claude Code**）能够搜索和查询你的知识库。
...
可以！将所有 API 地址指向本地 Ollama 服务即可：

- Embedding：`http://localhost:11434`，模型如 `nomic-embed-text`
- LLM：`http://localhost:11434`，模型如 `llama3.2`
