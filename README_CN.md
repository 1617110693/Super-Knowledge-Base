...
- **联网搜索**：免费 DuckDuckGo 集成（无需 API Key），支持 Tavily/SearXNG 切换，Jina 代理回退
- **即时渲染流式传输**：Streamdown (Vercel) — 流式过程中增量解析 Markdown/LaTeX/数学公式，不因全量重渲染导致 OOM
- **延迟持久化**：流式输出期间跳过磁盘写入（仅完成时落盘），消除 I/O 卡顿
- **知识库多选**：支持同时检索多个知识库，访问隔离；取消全选时自动禁用 KB 工具
- **引用标注**：回复带 `[N]` 上标标记与 `[M-N]` 范围标记，点击预览原文分块（支持前后翻页 + "查看完整文档"按钮定位到原文）
- **思考过程**：可折叠显示推理过程（DeepSeek-R1 等），流式时自动滚动，完成后自动折叠
- **图片引用**：LLM 可引用文档中的图片 — 渲染为可点击缩略图，支持全屏预览
- **会话操作**：重命名、删除（侧边栏 + 悬浮框双入口）、重新生成、复制消息
- **标签页系统**：桌面端多标签页 — 对话标签、文档标签、知识库标签，支持键盘快捷键和"重新打开已关闭标签"
- 代码块复制、数学公式渲染（KaTeX）、自动滚动开关、一键滚动到底部
- **侧边栏悬浮框**：侧边栏收起时点对话图标 → 浮出会话列表，支持新对话和逐条删除
...
- **可折叠侧边栏**：知识库 + 对话记录分区，收起时图标按钮弹出悬浮框
- **标签页系统**：多标签页管理，支持关闭右侧、关闭其他、重新打开已关闭标签、键盘快捷键（Ctrl+Tab / Ctrl+Shift+Tab）
- 设置面板：导航标签页分类（通用/模型/对话/数据），测试连接按钮、独立开关
